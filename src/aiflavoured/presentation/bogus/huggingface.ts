import { HfInference, TextGenerationInput } from "@huggingface/inference";
...
export const hf = async () => {
  ...
  const api = process.env.huggingFaceAPIKey;
  const inference = new HfInference(api);

  const task = "summarization";
  const model = "facebook/bart-large-cnn";

  // let instance = await pipeline(task, model);
...
//   let result = await instance(article);

  // let result = await reviewer('I love transformers!');
