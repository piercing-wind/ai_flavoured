...
const titleSlide = async (pptx: pptxgen,font : Font, waterMark : boolean) => {
  ...
  const subtitlex = 3.42;
  const subtitley = 2.75;
  const subtitlew = 9.02;
  const subtitleh = 3.73;
...
const titleAndContent = async (pptx: pptxgen, font :Font, waterMark : boolean) => {
      ...
      //leftbody
      const leftBodyx = 1.35;
      const leftBodyy = 2.75;
      const leftBodyw = 5.32;
      const leftBodyh = 3.73;
      ...
      //rightbody
      const rightBodyx = 6.77;
      const rightBodyy = 2.75;
      const rightBodyw = 5.67;
      const rightBodyh = 3.73;
...
const sectionHeader = async (pptx: pptxgen, font: Font, waterMark: boolean) => {
  ...
  // Subtitle section settings
  const subtitlex = 0.75;
  const subtitley = 1.67;
  const subtitlew = 9.98;
  const subtitleh = 5.08;
...
const twoContent = async (pptx: pptxgen, font : Font, waterMark : boolean) => {
  ...
  const leftContentx = 0.75;
  const leftContenty = 2.77;
  const leftContentw = 5.6;
  const leftContenth = 3.55;
  const leftContentFontSize = 20;
  const leftContentFontFace = bodyFontFace;
  const leftContentAlign = "left";
  const leftContentValign = "top";

  // Right content section settings
  const rightContentx = 6.99;
  const rightContenty = 2.77;  
  const rightContentw = 5.6;
  const rightContenth = 3.55;
  const rightContentFontSize = 20;
...
export const biomePresentationTheme = async ({author, title, pptxData, imageSearch, waterMark}: PresentationData , photosWithLink : PresentationImage) => {
      ...
      //   const base64Images = await varientsToBase64(templates[templatePicker]) as Base64Image[] ; 
        ...
        const varient  =  themes.biomeTheme;  
        
        const base64Images = await localVarientsToBase64(varient) as  Localbase64Image[]
        
       
        const font : Font = {title: "Grandview Display (Headings)", body: "Grandview Display (Body)"};
