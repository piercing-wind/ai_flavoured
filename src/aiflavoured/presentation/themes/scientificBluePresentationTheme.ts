...
const titleSlide = async (pptx: pptxgen,font : Font, waterMark : boolean) => {
  ...
  const subtitlex = 7.37;
  const subtitley = 0.86;
  const subtitlew = 5.38;
  const subtitleh = 5.48;
  const subtitleFontSize = 32;
  const subtitleFontFace = bodyFontFace;
  const subtitleAlign = "left";
  const subtitleValign= "middle";
...
const titleAndContent = async (pptx: pptxgen, font :Font, waterMark : boolean) => {
      ...
      //leftbody
      const leftBodyx = 3.62;
      const leftBodyy = 2.7;
      const leftBodyw = 4.35;
      const leftBodyh = 4.02;
      ...
      //rightbody
      const rightBodyx = 8.39;
      const rightBodyy = 2.7;
      const rightBodyw = 4.44;
      const rightBodyh = 4.02;
...
const sectionHeader = async (pptx: pptxgen, font: Font, waterMark: boolean) => {
  ...
  // Subtitle section settings
  const subtitlex = 0.35;
  const subtitley = 0.63;
  const subtitlew = 12.33;
  const subtitleh = 2.99;
  const subtitleFontSize = 32;
  const subtitleFontFace = bodyFontFace;
  const subtitleAlign = "center";
  const subtitleValign = "bottom";
...
const twoContent = async (pptx: pptxgen, font : Font, waterMark : boolean) => {
  ...
  const leftContentx = 0.89;
  const leftContenty = 2.7;
  const leftContentw = 7.99;
  const leftContenth = 3.75;
  ...
  // Right content section settings
  const rightContentx = 9.18;
  const rightContenty = 2.7;  
  const rightContentw = 3.12;
  const rightContenth = 3.75;
...
export const scientificBluePresentationTheme = async ({author, title, pptxData, imageSearch, modelForColorAndTitle, waterMark}: PresentaionData) => {
      ...
      //   const base64Images = await varientsToBase64(templates[templatePicker]) as Base64Image[] ; 
        ...
        const varient  =  themes.scientificBluePresentationTheme;  
        
        const base64Images = await localVarientsToBase64(varient) as  Localbase64Image[]
        
       
        const font : Font = {title: "Biome (Headings)", body: "Biome (Headings)"};
        ...
        const lineSpacing: number = 40;
          ...
          case "intro":
            ...
            if (Array.isArray(bodyIntro)) {
              ...
              if(bodyIntro.length > maxCharCountForBody){
                let bodySHString = bodyIntro.replace(/\n/g, '\n\n');
                slideIntro.addText(bodySHString, {
                  color : colors.body,
                  placeholder: "body",
                  lineSpacing :lineSpacing
                });}else{
